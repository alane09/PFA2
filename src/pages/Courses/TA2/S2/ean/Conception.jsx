  ...
  const getCourses = async () => {
    try {
      ...
      const filteredCourses = response.data.filter(course => (
        course.matiere === "conception" && course.examen === 0
  ...
  const getExams = async () => {
    try {
      ...
      const filteredExams = response.data.filter(course => (
        course.matiere === "conception" && course.examen === 1
      ));
    ...
    <body>
      <main>
        <div className='container'>
          <h1 style={{ textAlign: 'center' }}><strong> Conception des systèmes électroniques </strong></h1>
        </div>
        <br />
