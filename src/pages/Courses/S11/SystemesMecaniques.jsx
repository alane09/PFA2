    ...
    const getCourses = async () => {
        try {
            ...
            const filteredCourses = response.data.filter(course => (
                course.matiere === "robotique" && course.examen === 0 && course.id !== 90
    ...
    const getExams = async () => {
        try {
            ...
            const filteredExams = response.data.filter(course => (
                course.matiere === "robotique" && course.examen === 1
            ));
        ...
        <body>
            <main>
                <div className='container'>
                    <h1 style={{ textAlign: 'center' }}><strong> Systèmes mécaniques pour la Robotique</strong></h1>
                </div>
...
export default SystemesMecaniques;
